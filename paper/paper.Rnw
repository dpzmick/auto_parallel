...
Clojure built ins such as \code{pmap}\footnotemark[1] and reducers\footnotemark[2] library provide data parallel sequence manipulation and transformation functions.
...
Each thread in the pool maintains a queue of tasks to work on.
...
Each call to \code{future} spawns a thread to compute the result of a function call in the background.
When a user is ready to access the computed value, they can \code{deref} the future to it.
\code{deref} will block until the value is computed.
For a user, \code{future} initially seem like a fairly natural way to parallelize recursive functions.
Unfortunately, \code{future} only works well when a small number of tasks are created, or when the tasks do a lot of blocking I/O.
...
Their future implementation executes tasks created with \code{future} in a fixed sized thread pool, but, the \code{future} task will block the thread executing it if the task creates a new task, then immediately \code{deref}s it.
This means that a programmer must be very careful not to create tasks in a Claypoole thread pool which depend on other tasks, otherwise their code may deadlock.
...
So, \parlet{} forms can be nested; a programmer does not have to worry if functions called in the bindings of a \parlet{} form also use \parlet{}.
...
For an example see Figure \ref{fig:fib_parfun}.
...
There is so upside to this; it eliminates the stack depth limit (which is an issue in Clojure because Clojure cannot implement tail call optimization), but the large number of allocations creates a large amount garbage collector pressure.
